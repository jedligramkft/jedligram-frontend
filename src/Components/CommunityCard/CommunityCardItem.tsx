		...
		>
			{threads[0]?.header && (
				<div
					...
					style={{ backgroundImage: `url(${threads[0].header})` }}
				/>
			)}

			<div
				className={`absolute inset-0 ${threads[0]?.header ? "bg-black/30" : ""}`}
			/>
					...
					<div className="shrink-0 flex h-16 w-16 items-center justify-center rounded-2xl bg-white/10 ring-1 ring-white/15">
						{threads[0]?.image ? (
							...
							<span className="text-2xl font-bold text-white/60">
								{threads[0]?.name.charAt(0).toUpperCase() ??
									"?"}
							</span>
						)}
					</div>
					<div className="flex-1">
						...
						<p className="text-sm text-white/70">
							{threads[0]?.description ||
								t("allCommunities.default_description")}
						</p>
					</div>
				</div>
				<div className="mt-auto flex items-center justify-end">
					<div className="rounded-full border border-white/20 px-4 py-2 text-xs font-semibold text-white transition hover:border-white/40 hover:bg-white/10">
