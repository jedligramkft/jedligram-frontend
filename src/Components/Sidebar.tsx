    ...
    <>
      ...
      <div className={`fixed top-0 left-0 h-screen w-48 md:w-60 z-50 md:z-40 flex flex-col pt-20 pb-4 items-center transform transition-transform duration-300 ease-in-out bg-linear-to-r from-[#1a1d23] to-[#2a2d31] border-r border-gray-700 overflow-y-auto overflow-x-hidden sidebar-scroll ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}>
        <div className="my-4 w-full px-3">
          ...
          <Link to="/all-communities" onClick={() => handleClick(-1)} className={`flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-white/90 transition hover:bg-white/10 ${activeCommunity === -1 ? "bg-white/10" : ""}`}>
          ...
          <>
            ...
            <div className="my-4 w-full px-3">
              <Link to="/create-community" onClick={closeSidebar} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-white/90 transition hover:bg-white/10">
                <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-sm font-bold">+</div>
                <div className="min-w-0">
              ...
              <>
                ...
                <div className="my-4 w-full px-3">
                  ...
                  <div className="flex flex-col gap-2">
                    {recentThreads.map((t) => (
                      <Link key={t.id} to={`/communities/${t.id}`} onClick={() => handleClick(t.id)} className={`flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-white/90 transition hover:bg-white/10 ${activeCommunity === t.id ? "bg-white/10" : ""}`}>
                        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-sm font-bold">#{t.id}</div>
                        <div className="min-w-0">
                          <p className="truncate text-sm font-semibold">{t.name || `Közösség #${t.id}`}</p>
                          <p className="text-xs text-white/60">Legutóbb megnyitva</p>
                        </div>
                      </Link>
                    ))}
                  </div>
                ...
                <div className="flex flex-col gap-2">
                  {joinedThreadIds.map(t => (
                    <Link key={t.id} to={`/communities/${t.id}`} onClick={()=>handleClick(t.id)}
                          className={`flex items-center gap-3 rounded-2xl border border-white/10 bg-white/5 px-3 py-2 text-white/90 transition hover:bg-white/10 ${activeCommunity===t.id?"bg-white/10":""}`}>
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-sm font-bold">#{t.id}</div>
                      <div className="min-w-0">
