...
const Navbar = ({ toggleSidebar, isLoggedIn }: NavbarProps) => {
	...
	return (
		<nav className="bg-linear-to-r from-[#1a1d23] to-[#2a2d31] backdrop-blur-md border-b border-gray-700 text-white shadow-lg sticky top-0 z-50 poppins-regular">
			<div className="sm:px-6 md:px-12 py-4 flex items-center justify-between relative">
				<div className="flex items-center">
					<button
						...
						className="mr-4 p-2 rounded-lg hover:bg-white/10 transition text-white md:hidden"
					>
						<svg
							className="h-6 w-6"
							viewBox="0 0 24 24"
				...
				<div className="hidden md:flex items-center space-x-4">
					<button
						...
						aria-label={activeTheme === "dark" ? "Világos téma" : "Sötét téma"}
						title={activeTheme === "dark" ? "Világos téma" : "Sötét téma"}
					>
						{activeTheme === "dark" ? (
							<svg
								className="h-5 w-5"
					...
					<div
						className="flex items-center gap-2 cursor-pointer hover:bg-white/10 px-4 py-2 rounded-lg text-white font-semibold transition"
					...
					{isLoggedIn && (
						<div
							className="flex items-center gap-2 cursor-pointer hover:bg-white/10 px-2 py-2 rounded-lg text-white font-semibold transition"
							onClick={() => navigate("/profile")}
						>
							<Link
								to="/profile"
								className="text-gray-300 hover transition font-medium"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
					...
					{!isLoggedIn && (
						<Link
							to="/auth/login"
							className="keep-white text-white bg-linear-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 px-6 py-2 rounded-lg font-semibold transition shadow-md"
						>
							Bejelentkezés
						</Link>
					)}
			...
			<div
				className={`md:hidden overflow-hidden transition-[max-height,opacity] duration-500 ease-in-out ${isOpen ? "max-h-72 opacity-100" : "max-h-0 opacity-0"}`}
			>
				<div className="px-6 pb-4 flex flex-col gap-3">
					<button
						type="button"
						onClick={() => setActiveTheme(toggleTheme())}
						className="flex items-center gap-2 text-left text-gray-300 hover:text-white transition font-medium"
					>
					...
					<Link
						to="/create-community"
						className="text-left text-gray-300 hover:text-white transition font-medium"
					>
						Közösség létrehozása
					</Link>
					<a
					...
					{!isLoggedIn && (
						<Link
							to="/auth/login"
							className="keep-white text-white bg-linear-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 px-4 py-2 rounded-lg font-semibold transition shadow-md text-left"
						>
							Bejelentkezés
						</Link>
					)}
