			...
			<div className="relative z-10 mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 pb-20 pt-16 md:px-12">
				...
				<div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
					{threads.slice(0, 6).map((thread) => (
						...
						>
							<div
								className={`absolute inset-0 bg-linear-to-br from-cyan-400/30 to-blue-500/2 opacity-0 transition duration-500 group-hover:opacity-100`}
							/>
							<div className="relative z-10 flex h-full flex-col gap-4">
								<div className="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.2em] text-white/50">
									{/* <span>{thread.name}</span> */} {/* KATEGÓRIA VOLT */}
								...
								<div className="mt-auto flex items-center justify-between">
									<span className="text-sm font-semibold text-white/80">
										{/* TODO Tagok számának megjelenítése majd ha benne lesz a backendben */}
										10 tag
										{/* {thread.members} */}
									</span>
				...
				<Link
					to="/all-communities"
					className="mx-auto mt-8 block rounded-full border border-white/20 px-6 py-3 text-sm font-semibold text-white transition hover:border-white/40 hover:bg-white/10"
				>
					További közösségek
				</Link>
			</div>
