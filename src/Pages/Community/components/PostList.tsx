  ...
  return (
    <div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl shadow-black/30 backdrop-blur">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-white">{t("community.post_list.feed")}</h2>
        {props.isJoined ? (
          <Link to={props.id ? `/communities/${props.id}/posts/new` : "/all-communities"} className="rounded-xl border border-white/20 bg-white/5 px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/10">
            {t("community.post_list.create_post")}
          </Link>
        ) : (
          <button onClick={props.onJoin} className="rounded-xl border border-white/20 bg-white/5 px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/10">
            {t("community.post_list.join_to_post")}
          </button>
        )}
      </div>
      <div className="mt-5 space-y-4">
        {props.posts.length === 0 && <div className="text-sm text-white/70">{t("community.post_list.no_posts")}</div>}
            ...
            >
              {isOpen && (
                <CommentSection
                  ...
                  commentDraftByPostId={props.commentDraftByPostId}
                  onSubmitComment={props.onSubmitComment}
                  setCommentDraft={props.setCommentDraft}
                  replyCommentsByCommentId={props.replyCommentsByCommentId}
                  openRepliesCommentId={props.openRepliesCommentId}
                  openReplyComposerCommentId={props.openReplyComposerCommentId}
                  replyDraftByCommentId={props.replyDraftByCommentId}
