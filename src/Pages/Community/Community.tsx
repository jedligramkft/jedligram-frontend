...
const Community = ({ isLoggedIn }: CommunityProps) => {
	...
	const [replyCommentsByCommentId, setReplyCommentsByCommentId] = useState<
		Record<number, any[]>
	>({});
	...
	const [replyDraftByCommentId, setReplyDraftByCommentId] = useState<
		Record<number, string>
	>({});
	const [submittingReplyCommentId, setSubmittingReplyCommentId] = useState<
		number | null
	>(null);
	...
	const updateJoinedThreadsLocal = (
		threadIdValue: number,
		threadLabel: string,
		shouldBeJoined: boolean,
	) => {
		const profile = readProfile();
		const joinedThreads: string[] = Array.isArray(profile.joinedThreads)
		...
		localStorage.setItem(
			...
			JSON.stringify({
				...profile,
				joinedThreads: nextLabels,
				joinedThreadIds: nextIds,
			}),
		);
	...
	const syncLikeCountsFromPosts = (
		postsArray: Array<Record<string, unknown>>,
	) => {
		setLikeCounts((prev) => {
			const next = { ...prev };
	...
	const fetchJoinedUsernames = async (
		threadIdValue: number,
	): Promise<string[]> => {
	...
	const handleJoin = async () => {
		...
		try {
			...
			updateJoinedThreadsLocal(
				threadId,
				thread?.name ?? `Közösség #${threadId}`,
				true,
			);
			...
			console.log("Error joining thread:", err);
	...
	const loadCommentsForPost = async (postId: number) => {
		...
		try {
			...
			setLoadingCommentsPostId((current) =>
	...
	const handleSubmitComment = async (postId: number) => {
		...
		try {
			...
			setSubmittingCommentPostId((current) =>
				current === postId ? null : current,
			);
		}
	};
	...
	const loadRepliesForComment = async (commentId: number) => {
		...
		try {
			...
			setReplyCommentsByCommentId((prev) => ({
				...prev,
				[commentId]: Array.isArray(replyData) ? replyData : [],
			}));
		} catch (err) {
			console.error("Nem sikerült betölteni a válaszkommenteket", err);
		} finally {
			setLoadingRepliesCommentId((current) =>
	...
	const handleToggleReplyComposer = (commentId: number) => {
		if (Number.isNaN(commentId)) return;
		setOpenReplyComposerCommentId((current) =>
		...
		try {
			...
			if (created && typeof created === "object") {
				setReplyCommentsByCommentId((prev) => {
					...
					if (
						createdId !== undefined &&
						currentReplies.some((r: any) => r?.id === createdId)
					) {
						return prev;
					}
					return {
			...
			setSubmittingReplyCommentId((current) =>
				current === commentId ? null : current,
			);
		}
	};
	...
	const handleInvite = async () => {
		...
		try {
			...
			const message =
				err instanceof Error
					? err.message
					: "Nem sikerült megosztani a meghívót.";
			alert(message);
		}
	};
					...
					<div className="relative z-10 p-8 md:p-10">
						<div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
							...
							<div className="flex flex-wrap items-center gap-3">
								...
								<Link
									to="/all-communities"
									className="rounded-xl border border-white/15 px-5 py-2.5 text-sm font-semibold text-white/80 transition hover:border-white/30 hover:bg-white/10"
								>
									Vissza
								</Link>
							</div>
							...
							<div className="mt-5 space-y-4">
								...
								{!isLoading && posts.length === 0 && (
									<div className="text-sm text-white/70">
										Nincs még poszt ebben a közösségben.
									</div>
								)}
						...
						<div className="rounded-3xl border border-white/10 bg-white/5 p-6 shadow-xl shadow-black/30 backdrop-blur">
							...
							<div className="mt-4 space-y-3">
								{joinedUsernames.length === 0 ? (
									<div className="text-sm text-white/70">
										Nincsenek tagok ebben a közösségben.
									</div>
								) : (
									(showAllMembers
										? joinedUsernames
