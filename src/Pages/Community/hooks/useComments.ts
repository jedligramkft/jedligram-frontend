...
export const useComments = (isLoggedIn: boolean) => {
	...
	const handleSubmitComment = async (postId: number) => {
		...
		try {
			await CommentOnPostOrReplyToComment(postId, content);
	...
	const handleSubmitReply = async (postId: number, commentId: number) => {
		...
		try {
			await CommentOnPostOrReplyToComment(postId, content, commentId);
	...
	return {
		...
		handleSubmitComment,
		setCommentDraft: (postId: number, value: string) =>
			setCommentDraftByPostId((p) => ({ ...p, [postId]: value })),
		handleToggleReplies,
		handleReplyClick,
		handleSubmitReply,
		setReplyDraft: (commentId: number, value: string) =>
			setReplyDraftByCommentId((p) => ({ ...p, [commentId]: value })),
	};
};
