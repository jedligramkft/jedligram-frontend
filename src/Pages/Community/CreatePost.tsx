        ...
        <div className="relative mx-auto max-w-2xl overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-xl shadow-black/30 backdrop-blur">
          ...
          <div className="relative z-10 p-8 md:p-10">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              <div>
                <h1 className="text-2xl font-bold text-white">{t('createPost.title')}</h1>
                <p className="mt-2 text-sm text-white/70">{t('createPost.description')}</p>
                <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-white/15 bg-black/10 px-4 py-2 text-xs font-semibold text-white/80">
                  <span className="text-white/60">{t('createPost.community_label')}</span>
                  <span className="text-white/90">#{id ?? "—"}</span>
                </div>
              </div>

              <button type="button" onClick={() => navigate(id ? `/communities/${encodeURIComponent(id)}` : "/all-communities")}
            ...
            <form className="mt-8 grid gap-5" onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
              <div>
                <label className="text-xs font-semibold uppercase tracking-wider text-white/60">{t('createPost.content_label')}</label>
                <textarea placeholder={t('createPost.content_placeholder')} className="mt-2 h-44 w-full resize-none rounded-xl border border-white/10 bg-white/10 px-4 py-3 text-sm text-white placeholder:text-white/50 outline-none focus:border-white/20" value={content} onChange={(e) => setContent(e.target.value)}/>
              </div>

              <div className="flex items-center justify-between gap-4">
                <p className="text-xs text-white/55">{t('createPost.info_text')}</p>
                  ...
                  {creating ? t('createPost.submit_button_loading') : t('createPost.submit_button')}
                </button>
              </div>
            </form>
          </div>
